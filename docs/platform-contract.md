...
```json
{
  ...
  "team_front_door": { "user_pool_id": "af-south-1_...", "user_pool_arn": "arn:aws:cognito-idp:...", "domain": "<project>-<env>-team-<account>" },
...
| `team_front_door` | (the team-tools repository, not services) the Cognito user pool the tools' web addresses sit behind, and its sign-in domain prefix (`https://<domain>.auth.<region>.amazoncognito.com`). The tools repository creates its own app client (with its web addresses as callback URLs) and the client's managed login style, then puts an `authenticate-cognito` action in front of its load balancer rules. Null in production, whose tools are reached only through a tunnel |
