...
| MongoDB is accepted as a name and refused at plan | It runs on DocumentDB, whose module does not exist yet; the name is reserved so enabling it later changes no interface |
...
| The RDS administrator is `platform_admin` in every environment | RDS allows at most 16 characters; a name built from the project could exceed it |
