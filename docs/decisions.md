...
| Each team member gets their own database login, `agent_<name>`, from `data/people.json`: read or write in development and staging, read-only in production | Actions are traceable to a person, and removing someone is deleting one line. The prefix tells a person from a service (whose user is its name with underscores), and core refuses service names beginning `agent-` so the two cannot collide |
...
| The `agent_` logins are made to match the people secret on every engine at the end of every core apply, and again after every service is provisioned | One source of truth, applied in full each time: a person added, changed or removed is reflected everywhere, and a new service's database is covered the moment it exists. The staging and production functions take `{"action": "people"}`; the database host runs `provision-people.sh` through its own SSM document. Anyone who may invoke a function may ask for it; it only ever applies what core wrote to the secret |
| PostgreSQL gives access through two groups (`agent_group_read`, `agent_group_write`) with default privileges; MySQL grants each person directly on each service's database; MongoDB uses `readAnyDatabase` / `readWriteAnyDatabase` | Only default privileges cover tables a service creates later in PostgreSQL, and they are given to roles, so to groups. MySQL database-level grants already cover later tables, and granting a MySQL role needs `ROLE_ADMIN`, which the RDS master user may not have. DocumentDB supports the "any database" roles, so there is nothing per database. None can change tables: that stays with the services' migrations |
| The access level is kept in the people secret, not a parameter | The staging and production functions run in the isolated tier, whose only AWS endpoint is Secrets Manager |
