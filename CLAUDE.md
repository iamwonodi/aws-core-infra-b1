...
- Staging and production declare every engine and run only those in their `database_engines`. Each active engine gets its own provisioning function (PostgreSQL and MySQL tested against real servers with `PROVISION_TEST_PG_HOST` / `PROVISION_TEST_MYSQL_HOST`; DocumentDB offline only). `mongodb` runs on `terraform-aws-documentdb`, which must be readable by core's CI (public, like the other modules). MongoDB users live in `admin` in every environment. The internal tier is off by default everywhere (`internal_tier_enabled`): no internal load balancer, development's internal fleet at zero, and no `internal` entry in the contract or service roles. Staging's `database_schedule` is `always_on` (the default, as production always is) or `working_hours` (`modules/database/schedule`: EventBridge Scheduler starts the instances on the chosen days and stops them daily). The schedules have never run against AWS.
