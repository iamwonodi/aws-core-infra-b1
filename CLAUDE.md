...
- Staging and production declare every engine and run only those in their `database_engines`. Each active engine gets its own provisioning function (PostgreSQL and MySQL, both tested against real servers with `PROVISION_TEST_PG_HOST` / `PROVISION_TEST_MYSQL_HOST`). `mongodb` is refused until a DocumentDB module exists.
