...
`modules/` holds the four infrastructure domains — `network`, `edge`, `compute` (`image/` for the golden AMI and `deploy-bucket/`, both used by every environment), `database` (`host/` for development, `provisioning/` for the managed instance) — and `platform/`, which holds what is not infrastructure: `identity`, `service-roles`, `service-boundary`, `engines-role`, `contract`, `people` (the team's database logins and front door) and `host-scripts`. A module used by exactly one domain is nested inside it (`edge/cloudfront`, `network/nacl-security`).
...
(cd modules/platform/<module> && terraform init -backend=false && terraform test)     # identity, service-roles, service-boundary, contract, engines-role, people
...
- Staging and production declare every engine and run only those in their `database_engines`. Each active engine gets its own provisioning function (PostgreSQL and MySQL tested against real servers with `PROVISION_TEST_PG_HOST` / `PROVISION_TEST_MYSQL_HOST`; DocumentDB offline only). `mongodb` runs on `terraform-aws-documentdb`, which must be readable by core's CI (public, like the other modules). MongoDB users live in `admin` in every environment. Every provisioning connection verifies the server's certificate and host name against `modules/database/provisioning/lambda/certificates/rds-global-bundle.pem`, which must be downloaded from AWS and committed (its README); without it the plan and CI fail. Development and staging use a NAT instance (`nat_type`), production a NAT Gateway; the NAT instance and routing v1.1.0 modules must be published and public like the others. The internal tier is off by default everywhere (`internal_tier_enabled`): no internal load balancer, development's internal fleet at zero, and no `internal` entry in the contract or service roles. Staging's `database_schedule` is `always_on` (the default, as production always is) or `working_hours` (`modules/database/schedule`: EventBridge Scheduler starts the instances on the chosen days and stops them daily). The schedules have never run against AWS.
