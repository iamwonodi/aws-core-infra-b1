...
registry '{"postgres":{"port":20001,"active":true},"mysql":{"port":20002,"active":false}}'
out=$(U 2>&1); rc=$?
...
check "platform vars reach compose (port + data root)" bash -c "grep -qx 'ENGINE_PORT=20001' $FAKE_ROOT/up-env/db-postgres && grep -qx 'DATA_ROOT=$WORK/data/data' $FAKE_ROOT/up-env/db-postgres"
...
registry '{"mysql":{"port":20002,"active":false}}'
...
for case in \
  '{"a":{"port":20001},"b":{"port":20001}}|more than one active engine' \
  '{"a":{"port":80}}|invalid entry' \
  '{"a":{"port":"20001"}}|invalid entry' \
  '{"A_b":{"port":20001}}|invalid entry' \
  '{"a":{"port":20001,"active":"yes"}}|invalid entry' \
...
setup; engine a $IMG; engine b $IMG; registry '{"a":{"port":20001,"active":true},"b":{"port":20001,"active":false}}'
out=$(U 2>&1); rc=$?
check "same port allowed when only one is active"    test $rc -eq 0

echo "== engine-level guards"
setup; engine pg $IMG; registry '{"pg":{"port":20001}}'
out=$(U 2>&1); rc=$?
check "active omitted defaults to true"              test "$(cat $FAKE_ROOT/compose-state/db-pg 2>/dev/null)" = running
setup; engine pg $IMG '/var/lib/elsewhere:/var/lib/data'; registry '{"pg":{"port":20001}}'
out=$(U 2>&1); rc=$?
...
setup; engine pg postgres:16; registry '{"pg":{"port":20001}}'
out=$(U 2>&1); rc=$?
...
ENGINE_REQ=1; REQUIRE_ECR=false; export REQUIRE_ECR; setup; engine pg postgres:16; registry '{"pg":{"port":20001}}'
out=$(U 2>&1); rc=$?
check "Docker Hub image fine when not required"      test $rc -eq 0
unset REQUIRE_ECR
setup; engine pg $IMG 'PLACEHOLDER'; registry '{"pg":{"port":20001}}'
...
setup; engine pg $IMG; registry '{"pg":{"port":20001}}'; printf 'POSTGRES_PASSWORD=__FROM_SECRET__:CORE_ROOT_SECRET_ARN:nope\n' > $FAKE_ROOT/s3/b/database/engines/pg/.env
out=$(U 2>&1); rc=$?
...
setup; engine ok $IMG; engine bad $IMG; registry '{"ok":{"port":20001},"bad":{"port":20002}}'
...
setup; engine pg $IMG; mkdir -p $FAKE_ROOT/s3/b/database/engines/stray; printf '{}' > $FAKE_ROOT/s3/b/database/engines/stray/docker-compose.yaml; registry '{"pg":{"port":20001}}'
out=$(U 2>&1)
...
ENABLE_ECR=false; export ENABLE_ECR; setup; engine pg $IMG; registry '{"pg":{"port":20001}}'; out=$(U 2>&1)
