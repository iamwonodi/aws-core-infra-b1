output "service_roles" {
  ...
  value = local.enabled ? {
    (var.repository.name) = {
      ...
      description   = "CI role for the database engines repository in ${var.environment}."

      inline_policies = {
        "database-engines-access" = local.policy
      }
  ...
  depends_on = [terraform_data.engines_role_invariants]
}
