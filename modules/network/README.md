...
* **A single outbound (egress) rule** for public, private, internal, and vpc-endpoint security groups. Outbound traffic isn't this architecture's primary control point -- inbound rules and subnet routing are -- so this stays permissive by design. The isolated tier deliberately gets none: it has no NAT route to use one with, and reaches AWS services only through the VPC endpoints below.
* **VPC endpoints for the isolated tier** -- S3 (gateway endpoint, no cost), plus interface endpoints for ECR, SSM, Secrets Manager, and KMS, so the database can pull images, be managed via Session Manager, and fetch its credentials without ever needing a route out of the VPC.
