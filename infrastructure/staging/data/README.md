...
| Field | Meaning |
| --- | --- |
| `service_default` | Each service's own login (the one its application uses). Ships as `15` here |
...
**Caps are ceilings, not reservations.** This environment's engine is, by default (`database_instance_class`), `db.t4g.micro` (1 GiB): about 80 PostgreSQL connections, about 60-70 on MySQL. If the caps added together exceed that, services can still crowd each other out when every one is busy at once; the caps only stop any one of them taking it all.
